...
void ElectronDownloadManagerDelegate::SetDownloadManager(
...
bool ElectronDownloadManagerDelegate::DetermineDownloadTarget(
  ...
  FilenameDeterminedCallback filename_determined_callback = base::BindOnce(
      &ElectronDownloadManagerDelegate::OnDownloadPathGenerated,
  ...
  base::ThreadPool::PostTask(
      ...
      base::BindOnce(&ElectronDownloadManagerDelegate::GenerateFilename,
...
void ElectronDownloadManagerDelegate::GetNextId(content::DownloadIdCallback callback) {
  static uint32_t next_id = download::DownloadItem::kInvalidId + 1;
  std::move(callback).Run(next_id++);
}

// static
void ElectronDownloadManagerDelegate::GenerateFilename(
    const GURL& url,
...
void ElectronDownloadManagerDelegate::ChooseDownloadPath(
    uint32_t download_id,
    content::DownloadTargetCallback callback,
    const base::FilePath& suggested_path) {
  DCHECK_CURRENTLY_ON(content::BrowserThread::UI);
...
void ElectronDownloadManagerDelegate::SetDownloadBehaviorForTesting(
